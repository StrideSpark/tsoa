...
export const generateRoutes = async (
    ...
    switch (routesConfig.middleware) {
        case 'express':
            template = path.join(__dirname, '..', 'routeGeneration/templates/express.ts.hbs');
            break;
        case 'hapi':
            template = path.join(__dirname, '..', 'routeGeneration/templates/hapi.ts.hbs');
            pathTransformer = (path: string) => path;
            break;
        case 'koa':
            template = path.join(__dirname, '..', 'routeGeneration/templates/koa.ts.hbs');
            break;
        default:
            template = path.join(__dirname, '..', 'routeGeneration/templates/express.ts.hbs');
    }
