...
export class TypeResolver {
    ...
    public resolve(): Tsoa.Type {
        ...
        if (this.typeNode.kind === ts.SyntaxKind.UnionType) {
            ...
            const supportType = unionType.types.some(
                    ...
                    enums: unionType.types.map(type => {
                        ...
                        switch (literalType.kind) {
                            ...
                            case ts.SyntaxKind.FalseKeyword:
                                return 'false';
                            default:
                                return String((literalType as any).getText());
                        }
                    }),
        ...
        if (typeReference.typeName.kind === ts.SyntaxKind.Identifier) {
            if (typeReference.typeName.getText() === 'Date') {
                return this.getDateType(this.parentNode);
            }

            if (typeReference.typeName.getText() === 'Buffer') {
                return { dataType: 'buffer' } as Tsoa.Type;
            }

            if (
                typeReference.typeName.getText() === 'Array' &&
            ...
            if (
                typeReference.typeName.getText() === 'Promise' &&
                typeReference.typeArguments &&
                typeReference.typeArguments.length === 1
            ) {
            ...
            if (typeReference.typeName.getText() === 'String') {
    ...
    private getEnumerateType(typeName: ts.EntityName, extractEnum = true): Tsoa.Type | undefined {
        const enumName = (typeName as ts.Identifier).getText();
        const enumNodes = this.current.nodes
            .filter(node => node.kind === ts.SyntaxKind.EnumDeclaration)
            .filter(node => (node as any).name.getText() === enumName);
    ...
    private getLiteralType(typeName: ts.EntityName): Tsoa.OneOfType | undefined {
        const literalName = typeName.getText();
        const literalTypes = this.current.nodes
            ...
            .filter(node => (node as any).name.getText() === literalName);
        ...
        const typeOptions = unionTypes.map(unionType => {
            let referenceType: Tsoa.ReferenceType;
            if (unionType.typeArguments && unionType.typeArguments.length === 1) {
                const typeT: ts.NodeArray<ts.TypeNode> = unionType.typeArguments;
                referenceType = this.getReferenceType(unionType.typeName, this.extractEnum, typeT);
            } else {
                referenceType = this.getReferenceType(unionType.typeName, this.extractEnum);
            }
            return referenceType;
        });
    ...
    private getReferenceType(
        ...
        try {
            ...
            referenceType.properties = (referenceType.properties as Tsoa.Property[]).concat(
                properties
            );
            ...
            console.error(
                `There was a problem resolving type of '${this.getTypeName(
                    typeName,
                    genericTypes
                )}'.`
            );
    ...
    private resolveFqTypeName(type: ts.EntityName): string {
        if (type.kind === ts.SyntaxKind.Identifier) {
            return (type as ts.Identifier).getText();
        }

        const qualifiedType = type as ts.QualifiedName;
        return (
            this.resolveFqTypeName(qualifiedType.left) +
            '.' +
            (qualifiedType.right as ts.Identifier).getText()
        );
    }
    ...
    private getAnyTypeName(typeNode: ts.TypeNode): string {
        ...
        try {
            return (typeReference.typeName as ts.Identifier).getText();
        } catch (e) {
    ...
    private resolveModelTypeScope(leftmost: ts.EntityName, statements: any): any[] {
        while (leftmost.parent && leftmost.parent.kind === ts.SyntaxKind.QualifiedName) {
            const leftmostName =
                leftmost.kind === ts.SyntaxKind.Identifier
                    ? (leftmost as ts.Identifier).getText()
                    : (leftmost as ts.QualifiedName).right.getText();
            const moduleDeclarations = statements.filter(node => {
                if (
                    node.kind !== ts.SyntaxKind.ModuleDeclaration ||
                    !this.current.IsExportedNode(node)
                ) {
                    return false;
                }
                ...
                return (
                    (moduleDeclaration.name as ts.Identifier).getText().toLowerCase() ===
                    leftmostName.toLowerCase()
                );
            }) as ts.ModuleDeclaration[];

            if (!moduleDeclarations.length) {
                throw new GenerateMetadataError(
                    `No matching module declarations found for ${leftmostName}.`
                );
            }
            ...
            if (moduleBlock === null || moduleBlock.kind !== ts.SyntaxKind.ModuleBlock) {
                throw new GenerateMetadataError(
                    `Module declaration found for ${leftmostName} has no body.`
                );
            }
    ...
    private getModelTypeDeclaration(type: ts.EntityName) {
        ...
        const statements: any[] = this.resolveModelTypeScope(
            leftmostIdentifier,
            this.current.nodes
        );
        ...
        const typeName =
            type.kind === ts.SyntaxKind.Identifier
                ? (type as ts.Identifier).getText()
                : (type as ts.QualifiedName).right.getText();
        ...
        let modelTypes = statements.filter(node => {
            ...
            return (modelTypeDeclaration.name as ts.Identifier).getText() === typeName;
        ...
        if (modelTypes.length > 1) {
            ...
            const designatedModels = modelTypes.filter(modelType => {
                const isDesignatedModel = isExistJSDocTag(
                    modelType,
                    tag => tag.tagName.getText() === 'tsoaModel'
                );
    ...
    private getModelProperties(
        ...
        const isIgnored = (e: ts.TypeElement | ts.ClassElement) => {
            const ignore = isExistJSDocTag(e, tag => tag.tagName.getText() === 'ignore');
            return ignore;
        };
            ...
            return interfaceDeclaration.members
                ...
                .map((member: any) => {
                    ...
                    if (
                        ...
                        if ((typeIdentifier as ts.Identifier).getText()) {
                            typeIdentifierName = (typeIdentifier as ts.Identifier).getText();
                        } else {
                            typeIdentifierName = (typeIdentifier as ts.QualifiedName).right.getText();
                        }
        ...
        const properties = classDeclaration.members
            ...
            .filter(member => this.hasPublicModifier(member)) as Array<
        ...
        if (classConstructor && classConstructor.parameters) {
            const constructorProperties = classConstructor.parameters.filter(parameter =>
                this.hasPublicModifier(parameter)
            );
