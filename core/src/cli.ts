...
import { fsExists, fsMkDir, fsReadFile } from './utils/fs';
...
const validateSwaggerConfig = async (config: SwaggerConfig): Promise<SwaggerConfig> => {
    ...
    if (!(await fsExists(config.entryFile))) {
        throw new Error(
            `EntryFile not found: ${config.entryFile} - Please check your tsoa config.`
        );
    }
...
const validateRoutesConfig = async (config: RoutesConfig): Promise<RoutesConfig> => {
    ...
    if (
        config.iocModule &&
        !((await fsExists(config.iocModule)) || (await fsExists(config.iocModule + '.ts')))
    ) {
...
async function swaggerSpecGenerator(args) {
    try {
        ...
        if (config.swagger.version && config.swagger.version === '3.0.0') {
            spec = new SpecGenerator3(metadata, config.swagger).GetSpec();
        } else {
            spec = new SpecGenerator2(metadata, config.swagger).GetSpec();
        }

        const exists = fsExists(swaggerConfig.outputDirectory);
        if (!exists) {
            fsMkDir(swaggerConfig.outputDirectory);
        }
